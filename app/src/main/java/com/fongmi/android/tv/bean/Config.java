...
public class Config {
    ...
    public static Config vod() {
        ...
        return item == null ? createDefault(0, BuildConfig.DEFAULT_VOD_CONFIG) : item;
    }

    public static Config live() {
        Config item = AppDatabase.get().getConfigDao().findOne(1);
        return item == null ? createDefault(1, BuildConfig.DEFAULT_LIVE_CONFIG) : item;
    }

    public static Config wall() {
        Config item = AppDatabase.get().getConfigDao().findOne(2);
        return item == null ? createDefault(2, BuildConfig.DEFAULT_WALL_CONFIG) : item;
    }
    ...
    private static Config createDefault(int type, String url) {
        return TextUtils.isEmpty(url) ? create(type) : create(type, url, "Built-in");
    }
